...
class CAS_utils(object):
    ...
    def load_training_set_image_name(self):
        ...
        """
        filename = self.path + '\\Evaluation Prototype\\Training Set\\Training Set.txt'
        image_paths = []
        with open(filename, 'r') as file_to_read:
            while True:
                image_path = file_to_read.readline()
                if not image_path:
                    break
                image_paths.append((image_path[:-1] + '.tif'))
            # 将路径列表转换为numpy数组类型
    ...
    def load_gallery_image_name(self):
        ...
        - image_paths: 训练集的图片名称的一维数组 image_paths.shape = (N, )
        """
        filename = self.path + '\\Evaluation Prototype\\Gallery\\Gallery.txt'
        image_paths = []
        with open(filename, 'r') as file_to_read:
            while True:
                image_path = file_to_read.readline()
    ...
    def load_Probe_Sets_image_name(self, mode=None):
        ...
        filename = self.path +'\\Evaluation Prototype\\Probe Sets\\Probe Set_'
    ...
    """ 附加：
        ...
        image_paths = fnmatch.filter(os.listdir('C:/Users/吴宇翔/Desktop/CAS-PEAL-R1/FRONTAL/Normal'), '*.tif')
        list.sort(image_paths)
        n = len(image_paths)
        images = []
        print('Loading  images  ... ')
        
        # 进度条看读取的状态 for i, image_ in tqdm(enumerate(image_paths), total=len(image_paths)):
        for i, image_ in enumerate(image_paths):
            image = io.imread('C:/Users/吴宇翔/Desktop/CAS-PEAL-R1/FRONTAL/Normal/'+image_)
            images.append(image)
        print('Completed ... ')
    """
